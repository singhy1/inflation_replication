...
getOther <- function(data, year, month) {
  ...
  relevant_ids <- filtered_data$CPSIDP[filtered_data$WHYUNEMP %in% c(3)]
  relevant_weights <- filtered_data$WTFINL[filtered_data$WHYUNEMP %in% c(3)]
...
write_dta(transition_rates, file.path(proj_dir, "/temp/gross_flows_v2.dta"))
