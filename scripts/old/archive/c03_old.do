...
quietly infix                   ///
  ...
  byte    empsame      119-120  ///
  int     educ         121-123  ///
  double  earnwt       124-133  ///
  double  lnkfw1mwt    134-147  ///
  double  hourwage     148-152  ///
  byte    paidhour     153-153  ///
  double  earnweek     154-161  ///
  int     uhrsworkorg  162-164  ///
  byte    wksworkorg   165-166  ///
  using `"cps_00106.dat"'
...
* drop missing and top coded weekly earnings 
* drop if EARNWEEK2 == 9999.99
* drop if EARNWEEK2 > 2880


** trimming top and bottom 2% of all observations 
*drop pctile_earn2 pctile_earn98
* date 
...
/*
* Generate 3rd and 97th percentiles within each date_monthly
*by date_monthly: egen pctile_earn2 = pctile(EARNWEEK2), p(1)
*by date_monthly: egen pctile_earn99 = pctile(EARNWEEK2), p(99)

* Create a flag for observations to keep (between 2nd and 98th percentile)
gen keep_flag = (EARNWEEK2 >= pctile_earn2)

*gen keep_flag = (EARNWEEK2 >= pctile_earn2 & EARNWEEK2 <= pctile_earn98)

* Keep only the observations within the 2-98 percentile range
keep if keep_flag == 1

* Clean up
drop pctile_earn2 pctile_earn99 keep_flag
*/ 
...
* Age  
gen age_bin = .
replace age_bin = 1 if AGE >= 25 & AGE <= 34
replace age_bin = 2 if AGE >= 35 & AGE <= 44
replace age_bin = 3 if AGE >= 45 & AGE <= 55
label define age_bin_label 1 "25-34" 2 "35-44" 3 "45-55"
label values age_bin age_bin_label

* Education 
gen educ = .
replace educ = 1 if EDUC >= 111
replace educ = 0 if EDUC < 111
label define educ_label 0 "Less than College" 1 "College+"
...
label define race_label 0 "other" 1 "white"
label values white race_label

* Nativity
gen native = .
replace native = 1 if NATIVITY == 1  // Born in US states or territories
replace native = 0 if NATIVITY > 1 // Foreign born
label define native_label 0 "Foreign Born" 1 "US Born"
label values native native_label
...
* Create q1_2019 variable
gen q1_2019 = (date_monthly >= td(01jan2019) & date_monthly <= td(31mar2019))

* Logistic regression
logit q1_2019 i.age_bin i.educ i.white i.native i.female

* Generate propensity scores
predict propensity_score

* Calculate weights
gen weight = 1 / propensity_score if q1_2019 == 1
replace weight = 1 / (1 - propensity_score) if q1_2019 == 0

* weights 
sum weight
gen temp_wgt = weight / r(mean)


* cx weight represents the number of demographic people represented by the person 
gen cx_wgt = temp_wgt * WTFINL
replace cx_wgt = int(cx_wgt)

* earn weight represents the number of earning people represented by the person 
gen earn_wgt = temp_wgt * EARNWT
...
export delimited using "C:\Users\singhy\Dropbox (BFI)\Labor_Market_PT\temp\cps_data\cps_all_workers_82_89.csv", replace 
...
export delimited using "C:\Users\singhy\Dropbox (BFI)\Labor_Market_PT\temp\cps_data\cps_all_workers_90_99.csv", replace 
...
export delimited using "C:\Users\singhy\Dropbox (BFI)\Labor_Market_PT\temp\cps_data\cps_all_workers_00_09.csv", replace 
...
export delimited using "C:\Users\singhy\Dropbox (BFI)\Labor_Market_PT\temp\cps_data\cps_all_workers_10_19.csv", replace 

restore 

keep if YEAR >= 2020 & YEAR <= 2024 

export delimited using "C:\Users\singhy\Dropbox (BFI)\Labor_Market_PT\temp\cps_data\cps_all_workers_20_24.csv", replace 
*/ 
