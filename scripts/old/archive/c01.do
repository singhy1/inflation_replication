...
generate educ = cond(EDUC < 73, 1, ///
                cond(EDUC == 73, 2, ///
                cond(EDUC > 73 & EDUC <= 81, 3, ///
                cond(EDUC >= 90 & EDUC <= 92, 4, ///
                cond(EDUC > 92 & EDUC <= 122, 5, ///
                cond(EDUC > 122 & EDUC <= 125, 6, .))))))
...
4 "Associates" ///
5 "College grad" ///
6 "Masters, professional, doctoral"

* Apply the labels to the educ variable
label values educ educ_labels

* Optionally, you can also label the variable itself
