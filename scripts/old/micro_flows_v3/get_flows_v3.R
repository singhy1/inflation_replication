...
getLayoffs <- function(data, year, month) {
  ...
  relevant_ids <- filtered_data$CPSIDP[filtered_data$WHYUNEMP %in% c(1, 2)]
  relevant_weights <- filtered_data$WTFINL[filtered_data$WHYUNEMP %in% c(1, 2)]
...
getOther <- function(data, year, month) {
  ...
  relevant_ids <- filtered_data$CPSIDP[filtered_data$WHYUNEMP %in% c(3,4,5)]
  relevant_weights <- filtered_data$WTFINL[filtered_data$WHYUNEMP %in% c(3,4,5)]
...
write_dta(transition_rates, file.path(proj_dir, "/temp/gross_flows_v3.dta"))
